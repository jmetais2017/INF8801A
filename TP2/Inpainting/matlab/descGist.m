...
%            Angles utilisés pour le calcul du filtre de Gabor
           rotations = linspace(0, (2*pi)/descGist.nbOri, descGist.nbOri);

%            Boucle sur l echelle
           for scale = 1 : descGist.nbScales 
%               Boucle sur les angles 
               for angle = 1 : descGist.nbOri 


                 theta = rotations(angle);
                 lda = descGist.filterSize/(3^(descGist.nbScales-scale)) ;
                 sigmaX = descGist.filterSize/(3^(descGist.nbScales-scale));
                 sigmaY = descGist.filterSize/(2^(descGist.nbScales-scale));


                filters(:,:,scale,angle) = descGist.getGabor(theta,lda, sigmaX, sigmaY);
       ...
       function dst = descGist(src)
           ...
           for scale = 1 : descGist.nbScales 
               for angle = 1 : descGist.nbOri 
                   ...
                   dst.values(:,:,scale,angle) = imresize(abs(imfilter(src, descGist.filters(:,:,scale,angle))), [descGist.gridSize descGist.gridSize]); 
